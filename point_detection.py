...
_dfPartner = pandas.read_csv(_pathSBPartner + "test.csv")
...
#            if ( abs(_dfOrigin.loc[_pointOriginCount,"latitude"] - _dfPartner.loc[_dResultCount,'latitude']) < 0.05 ):

                if ( abs(_dfOrigin.loc[_pointOriginCount,"longitude"] - _dfPartner.loc[_dResultCount,'longitude']) < 0.05 and \
                     abs(_dfOrigin.loc[_pointOriginCount,"latitude"]  - _dfPartner.loc[_dResultCount,'latitude'] ) < 0.05):
